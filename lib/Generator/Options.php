...
final class Options
{
     ...
     */
    public static function parse(array $argv, array $env = [], ?Profile $profile = null): self
    {
        ...
        if (isset($flags['help'])) {
            return self::informational($profile, help: true);
        }

        if (isset($flags['version'])) {
            return self::informational($profile, version: true);
        }

        $warnings = [];

        $source = self::pick($values, $operands, 'source', '<source>', 0, $usage);
        $namespace = self::pick($values, $operands, 'namespace', '<namespace>', 1, $usage);
        $target = self::pick($values, $operands, 'target', '<target>', 2, $usage);
        ...
        $namespace = self::normalizeNamespace($namespace, $profile, $warnings);
        $warnings = [...$warnings, ...$profile->namespaceWarnings($namespace)];
    ...
    private static function informational(Profile $profile, bool $help = false, bool $version = false): self
    {
        return new self($profile, '', '', '', null, true, false, false, false, self::VERBOSITY_NORMAL, $help, $version, []);
    }
     ...
     */
    private static function tokenize(array $argv, string $usage): array
    {
        $values = [];
        $flags = [];
        $operands = [];
        ...
        for ($i = 1; $i < $count; $i++) {
            ...
            if (str_starts_with($arg, '--')) {
                ...
                $name = self::resolveLong($name, $usage);
                ...
                if (in_array($name, self::VALUE_OPTIONS, true)) {
                    ...
                    self::setValue($values, $name, $value, '--' . $name, $usage, $inline === null);
                    continue;
                }
            ...
            foreach ($letters as $position => $letter) {
                ...
                if (in_array($name, self::VALUE_OPTIONS, true)) {
                    ...
                    self::setValue($values, $name, $value, '-' . $letter, $usage);
                    continue;
                }
     ...
     */
    private static function resolveLong(string $name, string $usage): string
    {
    ...
    ): string
    {
        ...
        if ($value === null || $value === '') {
            throw GeneratorException::usage(sprintf('missing %s.', $label), [$usage]);
        }
     ...
     * @throws GeneratorException
     */
    private static function normalizeNamespace(string $namespace, Profile $profile, array &$warnings): string
    {
        ...
        if ($namespace === '' || preg_match('/^' . $segment . '(\\\\' . $segment . ')*$/', $namespace) !== 1) {
            throw GeneratorException::usage(
                ...
                [sprintf('quote it so the shell keeps the backslashes, e.g. \'%s\'.', $profile->exampleNamespace)],
            );
        }
        ...
        if (!str_contains($namespace, '\\') && preg_match_all('/\p{Lu}/u', $namespace) > 1) {
            $warnings[] = sprintf(
                'namespace "%s" contains no "\\" separator. If you meant a nested namespace, '
                    . 'quote it so the shell keeps the backslashes: \'%s\'.',
                $namespace,
                $profile->exampleNamespace,
            );
        }

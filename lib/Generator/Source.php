...
final class Source
{
    ...
    private function fetch(Options $options, array $env): string
    {
        ...
        if ($source === '-') {
            return $this->readStdin($options->profile);
        }
        ...
        if (preg_match('#^([a-zA-Z][a-zA-Z0-9+.-]*)://#', $source, $match) === 1) {
            ...
            if ($scheme === 'http' || $scheme === 'https') {
                return $this->request(self::withToken($source, $options->token), $options->profile, $env);
            }
     ...
     * @throws GeneratorException
     */
    private function readStdin(Profile $profile): string
    {
        ...
        if ($this->stdin === null && function_exists('posix_isatty') && @posix_isatty(STDIN)) {
            throw GeneratorException::fetch(
                ...
                [sprintf(
                    'pipe the document in, e.g. cat schemas.json | %s - \'%s\' %s',
                    $profile->program,
                    $profile->exampleNamespace,
                    $profile->exampleTarget,
                )],
            );
        }
     ...
     * @throws GeneratorException
     */
    private function request(string $url, Profile $profile, array $env): string
    {
        $options = [
            ...
            'headers' => [
                ...
                'User-Agent' => $profile->program . ' (' . Options::PACKAGE . ')',
            ],
        ];
        ...
        if ($status < 200 || $status > 299) {
            throw GeneratorException::fetch(
                ...
                self::statusHints($status, $profile),
            );
        }
     ...
     * @return list<string>
     */
    private static function statusHints(int $status, Profile $profile): array
    {
        if ($status === 401 || $status === 403) {
            return [
                sprintf('typed %s require a token. Pass --token=<key> or set $FLYO_TOKEN.', $profile->subject()),
            ];
        }

        if ($status === 404) {
            return [
                sprintf('typed %s live at https://api.flyo.cloud/nitro/v1/openapi/schemas; ', $profile->subject())

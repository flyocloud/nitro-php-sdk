 ...
 *
 * A `$ref` resolves to a generated class only when the referenced schema is in `$classes`. Each
 * kind decides what goes in there, because a class is only the right type where the SDK
 * hydrates a model at runtime; everywhere else the referenced schema is described inline.
 */
final class TypeMapper
{
    ...
    /**
     * @param array<string, string> $classes schema key => fully qualified generated class
     */
    ...
    public function propertyType(
        ...
        return $this->withNullability($this->type($schema, $path, $depth, $refs), $schema, $required);
    }

    /**
     * Like {@see propertyType()}, but renders the top level of an object as an `array{...}` shape.
     *
     * For the one place the SDK hands out a PHP array instead of an object: a model property
     * typed `array<string,mixed>`, which ObjectSerializer builds with `settype($data, 'array')`.
     * Only the top level is converted. The values below it are still decoded JSON, so a nested
     * object stays `object{...}`.
     *
     * @param array<string, mixed> $schema
     * @param list<string> $path property path, for warning messages
     */
    public function arrayShapeType(array $schema, array $path, bool $required): string
    {
        $node = $schema;
        $refs = [];

        if (isset($node['$ref']) && is_string($node['$ref'])) {
            $key = $this->document->refKey($node['$ref']);
            $node = $key === null ? null : $this->document->schema($key);

            if ($node === null) {
                // Only for its warning: the reference is dangling or not local.
                $this->ref($schema['$ref'], $path, 0, []);

                return $this->withNullability('array<string, mixed>', $schema, $required);
            }

            $refs = [(string) $key];
        }

        $type = $this->objectType($node, $path, 0, $refs, 'array{', 'array<string, mixed>');

        return $this->withNullability($type, $schema, $required);
    }
     ...
     * @param list<string> $refs
     */
    private function objectType(
        array $schema,
        array $path,
        int $depth,
        array $refs,
        string $open = 'object{',
        string $opaque = '\stdClass',
    ): string {
     ...
     * @param array<string, mixed> $schema
     */
    private function withNullability(string $type, array $schema, bool $required): string
    {
        if ($type === 'mixed') {
            return $type;
        }
    ...
    private function allOf(array $members, array $path, int $depth, array $refs): string
    {
        ...
        foreach ($members as $member) {
            ...
            $required = [...$required, ...Schema::requiredKeys($member)];
        }

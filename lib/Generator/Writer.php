...
final class Writer
{
    ...
    public function write(array $files, Options $options): WriteResult
    {
        ...
        $removed = $options->clean
            ? $this->removeStale($target, $options->profile->managedDirectories(), array_keys($files), $options->dryRun)
            : [];
     ...
     *
     * Only the directories the profile writes to are scanned, and never recursively: a
     * subdirectory the consumer created is none of this generator's business.
     *
     * @param list<string> $directories relative to the target, `''` for the target itself
     * @param list<string> $keep relative paths this run produced
     * @return list<string>
     */
    private function removeStale(string $target, array $directories, array $keep, bool $dryRun): array
    {
        $removed = [];

        foreach ($directories as $directory) {
            $dir = $directory === '' ? $target : $target . '/' . $directory;
            ...
            foreach (glob($dir . '/*.php') ?: [] as $path) {
                $relative = ($directory === '' ? '' : $directory . '/') . basename($path);

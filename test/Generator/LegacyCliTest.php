...
/**
 * The deprecated flyo-generate-blocks end to end, with streams injected so nothing reaches the
 * real stdout/stderr.
 *
 * Apart from the deprecation warning, everything here is exactly what the command did before
 * flyo-generate-types existed.
 */
class LegacyCliTest extends TestCase
{
    private const DEPRECATION = 'warning: flyo-generate-blocks is deprecated and will be removed in '
        . 'the next major release. Use flyo-generate-types, which also generates containers and '
        . "entities: https://github.com/flyocloud/nitro-php-sdk#-typed-schemas\n";
    ...
    public function testGeneratesFromALocalFile(): void
    {
        ...
        $this->assertSame(self::DEPRECATION, $err);
    ...
    /**
     * A deprecation is a warning, and warnings survive --quiet.
     */
    public function testQuietPrintsNothingButTheDeprecationOnSuccess(): void
    {
    ...
    public function testTheDeprecationNamesTheReplacement(): void
    {
        [, , $err] = $this->invoke([self::fixture('hero'), 'App\Blocks', $this->dir, '-q']);

        $this->assertStringContainsString('deprecated', $err);
        $this->assertStringContainsString('flyo-generate-types', $err);
    }

    /**
     * The whole point of keeping the command: a target generated before the deprecation still
     * passes --check afterwards. The fixtures are that committed output.
     */
    public function testCommittedOutputFromBeforeTheDeprecationIsStillCurrent(): void
    {
        [$status] = $this->invoke([
            self::fixture('full'),
            'Fixture\Full',
            dirname(__DIR__) . '/fixtures/blocks/full/expected',
            '--check',
        ]);
    ...
    public function testEntityAndContainerSchemasAreStillIgnored(): void
    {
        [$status, $out] = $this->invoke([
            dirname(__DIR__) . '/fixtures/types/site/openapi.json',
            'App\Blocks',
            $this->dir,
        ]);

        $this->assertSame(ExitCode::OK, $status);
        $this->assertStringContainsString('1 block -> ', $out);
        $this->assertSame(['BlockTeaser.php'], array_map('basename', glob($this->dir . '/*') ?: []));
    }
    ...
    {
        return dirname(__DIR__) . '/fixtures/blocks/' . $name . '/openapi.json';
    }

...
class NamingTest extends TestCase
{
    ...
    public function testCollisionAssignmentIsIndependentOfInputOrder(): void
    {
        ...
        $forward = Naming::blockClassNames([...$a, ...$b], new Warnings());
        $reverse = Naming::blockClassNames([...$b, ...$a], new Warnings());
     ...
     */
    public static function prefixedKeyNames(): array
    {
        return [
            'key carries the prefix' => ['Entity', 'EntityArticle', 'EntityArticle'],
            'key lacks the prefix' => ['Entity', 'Article', 'EntityArticle'],
            'key needs normalising' => ['Container', 'Container main-nav', 'ContainerMainNav'],
            'lowercase key' => ['Container', 'footer', 'ContainerFooter'],
            'key is only the prefix' => ['Entity', 'Entity', 'EntityUnnamed'],
            'key is empty' => ['Entity', '', 'EntityUnnamed'],
            // A reserved word is harmless once it carries the prefix.
            'reserved word' => ['Entity', 'list', 'EntityList'],
        ];
    }

    #[DataProvider('prefixedKeyNames')]
    public function testPrefixedKeyName(string $prefix, string $key, string $expected): void
    {
        $this->assertSame($expected, Naming::prefixedKeyName($prefix, $key));
    }

    public function testResolveNamesTheKindAndHowToFixItInTheCollisionWarning(): void
    {
        $warnings = new Warnings();

        $names = Naming::resolve(
            ['Entity news' => 'EntityNews', 'EntityNews' => 'EntityNews'],
            $warnings,
            'entity',
            'Rename one of the entity types in Flyo to remove the ambiguity.',
        );

        $this->assertSame(['Entity news' => 'EntityNews', 'EntityNews' => 'EntityNews2'], $names);
        $this->assertSame(
            ["2 entity schemas want the class name EntityNews:\n"
                . "  Entity news -> EntityNews\n"
                . "  EntityNews -> EntityNews2\n"
                . '  Rename one of the entity types in Flyo to remove the ambiguity.'],
            $warnings->all()
        );
    }

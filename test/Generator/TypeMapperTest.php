...
class TypeMapperTest extends TestCase
{
    ...
    // -- array shapes, for ContainerPage::getProperties() -----------------------------------

    public function testArrayShapeConvertsOnlyTheTopLevel(): void
    {
        $type = self::mapper()->arrayShapeType([
            'type' => 'object',
            'properties' => [
                'icon' => ['type' => 'string'],
                'image' => ['type' => 'object', 'properties' => ['source' => ['type' => 'string']]],
            ],
        ], ['field'], true);

        $this->assertSame(
            "array{\n    icon: string|null,\n    image: object{source: string|null}|null\n}",
            $type
        );
    }

    public function testArrayShapeIsNullableUnlessRequired(): void
    {
        $schema = ['type' => 'object', 'properties' => ['a' => ['type' => 'string']]];

        $this->assertSame('array{a: string|null}|null', self::mapper()->arrayShapeType($schema, ['f'], false));
        $this->assertSame('array{a: string|null}', self::mapper()->arrayShapeType($schema, ['f'], true));
    }

    /**
     * An object with no declared properties is what the SDK's own type already says it is.
     */
    public function testArrayShapeWithoutPropertiesIsAnOpenArray(): void
    {
        $this->assertSame(
            'array<string, mixed>',
            self::mapper()->arrayShapeType(['type' => 'object', 'properties' => []], ['f'], true)
        );
        $this->assertSame(
            'array<string, mixed>',
            self::mapper()->arrayShapeType(['type' => 'object', 'additionalProperties' => true], ['f'], true)
        );
    }

    public function testArrayShapeFollowsAReference(): void
    {
        $this->assertSame(
            'array{name: string|null}',
            self::mapper()->arrayShapeType(['$ref' => '#/components/schemas/Plain'], ['f'], true)
        );
    }

    /**
     * A dangling reference is reported, but must not narrow the getter to `mixed`: the SDK's own
     * `array<string, mixed>` is still true.
     */
    public function testArrayShapeWithADanglingReferenceStaysAnOpenArray(): void
    {
        $warnings = new Warnings();

        $this->assertSame(
            'array<string, mixed>',
            self::mapper($warnings)->arrayShapeType(['$ref' => '#/components/schemas/Nope'], ['f'], true)
        );
        $this->assertStringContainsString('not in the document', implode("\n", $warnings->all()));
    }
